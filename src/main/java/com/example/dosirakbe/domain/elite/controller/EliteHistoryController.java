...
@RequestMapping("/api/elite-histories")
public class EliteHistoryController {
     ...
     */
    @GetMapping("/user")
    public ResponseEntity<ApiResponse<List<EliteHistoryResponseDto>>> getEliteHistoryByUserId(@AuthenticationPrincipal CustomOAuth2User customOAuth2User) {
        ...
        return ApiResponseUtil.ok("사용자의 히스토리 조회 성공", history);
    }
     ...
     */
    @PostMapping("/record")
    public ResponseEntity<ApiResponse<EmptyResponse>> recordAnswer(@AuthenticationPrincipal CustomOAuth2User customOAuth2User,
        ...
        return ApiResponseUtil.created("문제 풀이 기록 및 업데이트 성공", null);//TODO
    }
    ...
    public ResponseEntity<ApiResponse<List<EliteHistoryResponseDto>>> getCorrectProblemsByUserId(@AuthenticationPrincipal CustomOAuth2User customOAuth2User) {
        Long userId = getUserIdByOAuth(customOAuth2User);
        List<EliteHistoryResponseDto> correctHistory = eliteHistoryService.findCorrectProblemsByUserId(userId);

        return ApiResponseUtil.ok("맞은 문제 조회 성공", correctHistory);
    }
     ...
     */
    @GetMapping("/user/incorrect")
    public ResponseEntity<ApiResponse<List<EliteHistoryResponseDto>>> getIncorrectProblemsByUserId(@AuthenticationPrincipal CustomOAuth2User customOAuth2User) {
        Long userId = getUserIdByOAuth(customOAuth2User);
        List<EliteHistoryResponseDto> incorrectHistory = eliteHistoryService.findIncorrectProblemsByUserId(userId);
        return ApiResponseUtil.ok("틀린 문제 조회 성공", incorrectHistory);
    }
